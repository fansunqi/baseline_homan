...
class HOMan(nn.Module):    # 这个就是model
    def __init__(
        self,
        translations_object,
        rotations_object,
        verts_object_og,
        faces_object,  # 这里有faces_obj
        translations_hand,
        rotations_hand,
        verts_hand_og,
        ref_verts2d_hand,
    ...
    def save_obj(self, fname):
        ...
        with open(fname, "w") as fp:
            ...
            print("union_obj saved")

    def save_hand_obj(self, fname):
        with open(fname, "w") as fp:
            hand_verts_combined = combine_verts([self.get_verts_hand()[0]])
            hand_verts_num = 0
            # for v in tqdm.tqdm(hand_verts_combined[0]):
            for v in hand_verts_combined[0]:
                fp.write(f"v {v[0]:f} {v[1]:f} {v[2]:f}\n")
                hand_verts_num += 1
            o = 1
            for face in self.faces[0]:
                # 需要检查face[0], facce[1], face[2]是不是都在hand_verts_combined里面
                if face[0] < hand_verts_num and face[1] < hand_verts_num and face[2] < hand_verts_num:
                    fp.write(
                        f"f {face[0] + o:d} {face[1] + o:d} {face[2] + o:d}\n")
            print("hand_obj saved")

    def save_object_obj(self, fname):
        with open(fname, "w") as fp:
            object_verts_combined = combine_verts([self.get_verts_object()[0]])
            object_verts_num = 0
            # for v in tqdm.tqdm(object_verts_combined[0]):
            for v in object_verts_combined[0]:
                fp.write(f"v {v[0]:f} {v[1]:f} {v[2]:f}\n")
                object_verts_num += 1
            o = 1
            for face in self.faces[0]:
                # 需要检查face[0], facce[1], face[2]是不是都在object_verts_combined里面
                if face[0] < object_verts_num and face[1] < object_verts_num and face[2] < object_verts_num:
                    fp.write(
                        f"f {face[0] + o:d} {face[1] + o:d} {face[2] + o:d}\n")
            print("object_obj saved")
